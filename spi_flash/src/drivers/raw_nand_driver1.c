...
LOG_MODULE_REGISTER(custom_qspi_nand, CONFIG_FLASH_LOG_LEVEL);
...
static int qspi_write_enable_disable(const struct device *dev,
					 bool write_protect);
...
static uint8_t get_status(){
	...
	nrf_qspi_cinstr_conf_t gf_cinstr_cfg = {
		.opcode = SPI_NAND_GF,
	...
	nrfx_err_t res = nrfx_qspi_cinstr_xfer(&gf_cinstr_cfg, &reg_cmd, data);
	if (res == NRFX_SUCCESS){
	return data;
	}
	else {
...
static uint8_t get_features(uint8_t register_select){
	...
	nrfx_err_t res = nrfx_qspi_cinstr_xfer(&gf_cinstr_cfg, &register_select, data);
	if (res == NRFX_SUCCESS){
	return data;
	}
	else {
...
static int set_features(uint8_t register_select, uint8_t data){
	nrf_qspi_cinstr_conf_t gf_cinstr_cfg = {
		.opcode = SPI_NAND_GF,
	...
	nrfx_err_t res = nrfx_qspi_cinstr_xfer(&gf_cinstr_cfg, data_arr, NULL);
	if (res == NRFX_SUCCESS){
	if (get_features(register_select) == data){
		return 1;
	}
	else{
					    ...
					    nrfx_err_t res)
{
	for (int x = 0; x < 100000; x++){
		if (get_status() % 2 == 0){
			LOG_DBG("cycles before done: %i", x);
			break;
		}
	}
}
...
static int qspi_erase(const struct device *dev, uint32_t addr, uint32_t size)
{
	...
	qspi_trans_lock(dev);
	rv = qspi_write_enable_disable(dev, false);
	if (rv != 0) {
	...
	int rv2 = qspi_write_enable_disable(dev, true);
...
static int qspi_nand_read(const struct device *dev, 
			  void *data, size_t len, off_t offset)
{
	...
	nrf_qspi_cinstr_conf_t cread_cinstr_cfg = {
	...
	nrf_qspi_cinstr_conf_t pread_cinstr_cfg = {
	...
	res = nrfx_qspi_cinstr_xfer(&pread_cinstr_cfg, &addr_buf, NULL);
	if (res != NRFX_SUCCESS) {
		LOG_DBG("read error: %x", res);
		goto out;
	}
	for (int x = 0; x < 100000; x++){
		if (get_status() % 2 == 0){
	...
	res = nrfx_qspi_lfm_start(&cread_cinstr_cfg);
	if (res != NRFX_SUCCESS) {
...
static int qspi_nand_write(const struct device* dev, void* data, size_t len, 
	off_t address){
	...
	nrf_qspi_cinstr_conf_t pl_cinstr_cfg = {
		.opcode = SPI_NAND_PL,
		.length = NRF_QSPI_CINSTR_LEN_1B,
		.io2_level = true,
		.io3_level = true,
	...
	nrf_qspi_cinstr_conf_t pe_cinstr_cfg = {
		.opcode = SPI_NAND_PE,
		.length = NRF_QSPI_CINSTR_LEN_1B,
		.io2_level = true,
		.io3_level = true,
	...
	res = nrfx_qspi_lfm_start(&pl_cinstr_cfg);
	if (res != NRFX_SUCCESS) {
		LOG_WRN("lfm_start: %x", res);
		return res;
	}

	res = nrfx_qspi_lfm_xfer(pl_addr_buf, NULL, sizeof(pl_addr_buf), false);
	if (res != NRFX_SUCCESS) {
		LOG_WRN("lfm_xfer addr: %x", res);
		return res;
	}
	...
	res = nrfx_qspi_lfm_start(&pe_cinstr_cfg);
	if (res != NRFX_SUCCESS){
		LOG_WRN("lfm_start: %x", res);
		return res;
	}

	res = nrfx_qspi_lfm_xfer(pe_addr_buf, NULL, sizeof(pe_addr_buf), true);
	if (res != NRFX_SUCCESS) {
		LOG_WRN("lfm_xfer addr: %x", res);
		return res;
	}
...
static int qspi_read_security(const struct device* dev){
	
	uint8_t security_register;
	

	const struct qspi_buf rx_buf = {
		.buf = &security_register,
		.len = 1
	};
	...
	int status = qspi_send_cmd(dev, &cmd, true);
	return (status < 0) ? status : security_register;
 ...
 */
static inline int qspi_nand_read_id(const struct device *dev)
{
	uint8_t id[2];
	...
	if (memcmp(qnc->id, id, 2) != 0) {
		LOG_ERR("id [%02x %02x] expect [%02x %02x]",
			id[0], id[1], id[2],
					  ...
					  void *dest, size_t size)
{
	...
	if (flash_middle != 0) {
		...
		res = qspi_nand_read(dev, dptr + dest_prefix, flash_middle,
				     addr + flash_prefix);
	...
	/* read prefix */
	if (flash_prefix != 0) {
		res = qspi_nand_read(dev, buf, WORD_SIZE, addr -
	...
	/* read suffix */
	if (flash_suffix != 0) {
		res = qspi_nand_read(dev, buf, WORD_SIZE * 2,
...
static int qspi_read(const struct device *dev, off_t addr, void *dest,
			 size_t size)
{
					...
					const void *sptr, size_t slen)
{
	 ...
	 * written back
	 */
	res = qspi_nand_read(dev, buf, sizeof(buf), addr);
	qspi_wait_for_completion(dev, res);

	if (res == NRFX_SUCCESS) {
		memcpy(buf, sptr, slen);
		res = qspi_nand_write(dev, buf, sizeof(buf), addr);
	}
...
static int qspi_write(const struct device *dev, off_t addr,
			  const void *src,
			  size_t size)
{
	if (!src) {
	...
	qspi_trans_lock(dev);
	res = qspi_write_enable_disable(dev, false);
	qspi_lock(dev);
	if (!res) {
		if (size < 4U) {
			...
			res = qspi_nand_write(dev, src, size, addr);
		}
	}
	qspi_unlock(dev);

	int res2 = qspi_write_enable_disable(dev, true);
...
static int qspi_nand_erase(const struct device *dev, off_t addr, size_t size)
{
	const struct qspi_nor_config *params = dev->config;
...
/*
	...
	int status = qspi_send_cmd(dev, &RSEcmd, true);
	

	const struct qspi_cmd RSTcmd = {
		.op_code = SPI_NOR_CMD_RESET_MEM
	};

	const struct qspi_cmd unprotect = {
		.op_code = SPI_NOR_CMD_ULBPR
	};

	qspi_send_cmd(dev, &RSTcmd, true);
	if (ret != 0) {
		return ret;
	}

	qspi_send_cmd(dev, &unprotect, true);

*/
	//qspi_write_enable_disable(dev, false);
	int status_register = get_status();
	printk("status register: %d \n", status_register);
	int configuration = get_features(REGISTER_CONFIGURATION);
	printk("configuration register: %d \n", configuration);
	int security_register = get_features(REGISTER_BLOCKLOCK);
...
static const struct flash_driver_api qspi_api = {
	.read = qspi_read,
	.write = qspi_write,
	.erase = qspi_nand_erase,
...
DEVICE_DT_INST_DEFINE(0, qspi_init, PM_DEVICE_DT_INST_GET(0),
